...
fun QuizArea(navController: NavHostController) {
    Surface(
        ...
        Column(
            modifier = Modifier.fillMaxSize(),
            verticalArrangement = Arrangement.Center,               //測驗垂直方向按鈕置中
            horizontalAlignment = Alignment.CenterHorizontally      //測驗水平方向按鈕置中
        ){
            //開始測驗按鈕
            Button(
                onClick = {  }, //如果有新寫好的kt檔，請在括弧裡則輸入此程式碼"navController.navigate("你取的檔名")"
                modifier = Modifier
                    .height(150.dp)     // 設置按鈕高度為 150dp
                    .width(200.dp)      // 設置按鈕寬度為 200dp
            ) {
                Text(
                    text = "開始測驗",
                    fontSize = 32.sp
